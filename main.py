...
# این خطا زمانی رخ می‌دهد که ربات بخواهد به یک چنل/گروه خصوصی یا حذف‌شده
# که دیگر به آن دسترسی ندارد پیام ارسال کند. مدیریت این خطا باعث می‌شود
# ربات کرش نکند یا متوقف نشود.
try:
...
async def send_all(client, text):
    ...
    for r in rows:
        ...
        try:
            ...
            # چت دیگر در دسترس نیست (خصوصی/حذف‌شده/خروج ربات).
            # از لیست ارسال همگانی حذف می‌شود تا دیگر باعث خطا نشود.
            print(
                f"⚠️ چت {chat_id} در پیام همگانی نادیده گرفته شد: "
                f"ChannelPrivateError (چت دیگر در دسترس نیست)."
            )
...
async def safe_reply(event, message):
    try:
        ...
        # چت/چنل دیگر در دسترس ربات نیست؛ فقط لاگ می‌شود و ربات ادامه می‌دهد.
        print(
            f"⚠️ این چت دیگر در دسترس ربات نیست (ChannelPrivateError): {exc}"
        )
        return None
...
async def handler(event):
    try:
        ...
        # چت دیگر در دسترس ربات نیست؛ فقط لاگ می‌شود، ربات کرش نمی‌کند.
        print(
            f"⚠️ ChannelPrivateError در پردازش پیام "
            f"(uid={getattr(event, 'sender_id', '?')}): {exc}"
        )
...
async def _process_message(event):
    ...
    # نکته مهم: simple_commands شامل دستورهای چندکلمه‌ای هم هست
    # (مثل «جمع سکه»، «کد دعوت»، «پرداخت وام»، «آمار بازی»، ...).
    # بررسی فقط first_word برای این دستورها همیشه False برمی‌گشت
    # چون کلمه اول (مثلاً «جمع») در لیست نیست، فقط عبارت کامل هست.
    # همین باعث می‌شد ربات بدون هیچ پاسخی ساکت بماند.
    is_command = (
        text in simple_commands
        or first_word in simple_commands
